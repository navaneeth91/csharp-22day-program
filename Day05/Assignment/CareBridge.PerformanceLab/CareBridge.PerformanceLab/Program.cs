...
while (true)
{
    ...
    Console.WriteLine("7.The Revenue-at-Risk Dashboard");
...
static void RevenueAtRiskDashboard()
{
    ...
    using (var db = new CareBridgeContext())
    {
        // Query with projection, grouping, and AsNoTracking
        ...
        // Compute Revenue-at-Risk (all non-Paid claims)
        ...
        {
            Console.WriteLine(row.Status + "      " + row.ClaimCount + "        " + row.TotalBilled + "        " + row.TotalReimbursed + "        " + row.Gap);
  ...
  static void CartesianExplosionDemo()
    {
        using var db = new CareBridgeContext();
        Console.WriteLine();
        Console.WriteLine("EAGER LOADING DEMO (Single Query)");
        Console.WriteLine("----------------------------");
        ...
        // Assert correctness
        Debug.Assert(encounterCount == 100);
        Debug.Assert(diagnosisCount == 300);
        Debug.Assert(claimCount == 300);
    }
